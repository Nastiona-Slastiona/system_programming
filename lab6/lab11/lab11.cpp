...
BOOL InitInstance(HINSTANCE hInstance, int nCmdShow)
{
    ...
    thread4 = CreateThread(NULL, 0u, (LPTHREAD_START_ROUTINE)MoveLine, &args4, 0, NULL);
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_PAINT:
    {
        ...
        if (args1.y != 0)
        {
            HPEN pen = CreatePen(BS_SOLID, 2, RGB(255, 192, 203));
            SelectObject(hdc, pen);
            MoveToEx(hdc, args1.x, args1.y, NULL);
            LineTo(hdc, args1.x + 50, args1.y);
            HPEN pen2 = CreatePen(BS_SOLID, 2, RGB(128, 218, 235));
            SelectObject(hdc, pen2);
            MoveToEx(hdc, args2.x, args2.y, NULL);
            LineTo(hdc, args2.x, args2.y + 50);
            MoveToEx(hdc, args4.x, args4.y, NULL);
            LineTo(hdc, args4.x, args4.y + 50);
            SelectObject(hdc, pen);
